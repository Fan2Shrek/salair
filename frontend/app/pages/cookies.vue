<script setup lang="ts">
    ...
    const loadData = async () => {
        const result = await useAsyncData(`cookies-${locale.value}`, () =>
            queryCollection('content').path(`/cookies.${locale.value}`).first()
        );
    ...
    useSeoMeta({
        title: computed(() => t('cookies_page.title')),
        description: computed(() => t('cookies_page.intro'))
    });
</script>
        ...
        <main class="w-full flex-grow">
            <section class="pt-19 max-w-7xl mx-auto w-full">
                <div class="my-12 md:my-24 max-w-3xl w-full mx-auto px-4 md:px-0">
                    <p class="text-brand-secondary font-semibold text-center">{{ $t('cookies_page.last_updated') }}</p>
